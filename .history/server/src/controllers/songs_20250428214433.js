...
exports.list = asyncHandler(async (req, res) => {
  ...
  // 4) (Optional) Enrich each song with its artist’s other titles
  const items = await Promise.all(
    rawSongs.map(async (song) => {
      const songObj = song.toObject();
      if (songObj.artist?._id) {
        const artistSongs = await Song.find({ artist: songObj.artist._id })
          .select("title");
        songObj.artist.songs = artistSongs;
      }
      return songObj;
