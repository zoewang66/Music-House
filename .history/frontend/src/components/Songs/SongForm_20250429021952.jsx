...
export default function SongForm({ mode }) {
  ...
  // 1) Load all artists (across pages) into the dropdown
  useEffect(() => {
    const loadAll = async () => {
      try {
        let page = 1,
          ...
          all = [];
        while (true) {
          const wrapper = await fetchArtists(page, "", limit);
          // wrapper may be array or { items, … } or { data: […] }
  ...
  useEffect(() => {
    ...
    fetchSongById(id)
      .then((res) => {
        ...
        setDuration(song.duration);
        setReleaseDate(song.releaseDate ? new Date(song.releaseDate) : null);
        setSelectedArtist(song.artist?._id || "");
      })
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // if user typed a new artist name instead of picking ID:
      ...
      const payload = { title, duration, releaseDate, artist: artistId };
      if (mode === "create") {
