...
export default function ArtistForm({ mode }) {
  ...
  const [selectedSongs, setSelectedSongs] = useState([]);  // currently chosen IDs
  const [originalSongs, setOriginalSongs] = useState([]);  // for diff’ing
  const [error, setError] = useState(null);

  // 1) Load every song into the MultiSelect
  useEffect(() => {
    const loadSongs = async () => {
      try {
        const wrapper = await fetchSongs();
        // wrapper might be an array, or { items, … }, or { data: […] }
        const raw =
          Array.isArray(wrapper)             ? wrapper :
          Array.isArray(wrapper.items)       ? wrapper.items :
          Array.isArray(wrapper.data)        ? wrapper.data :
          [];
        setAllSongs(raw.map((s) => ({ value: s._id, label: s.title })));
      } catch (e) {
  ...
  // 2) If editing, preload artist fields + their song IDs
  useEffect(() => {
    if (mode !== "edit") return;
    fetchArtistById(id)
      .then((res) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      // 3a) create or update the Artist record
      let artistId = id;
      if (mode === "create") {
        const res = await createArtist({ name, genre, bio });
        artistId = res.data._id;
      ...
      // 3b) disassociate songs the user removed
      const toRemove = originalSongs.filter((sid) => !selectedSongs.includes(sid));
      await Promise.all(toRemove.map((sid) => 
        updateSong(sid, { artist: null })
      ));

      // 3c) associate any newly-added songs
      const toAdd = selectedSongs.filter((sid) => !originalSongs.includes(sid));
      await Promise.all(toAdd.map((sid) => 
        updateSong(sid, { artist: artistId })
      ));
