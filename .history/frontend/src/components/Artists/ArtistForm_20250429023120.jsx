...
export default function ArtistForm({ mode }) {
  ...
  const [selectedSongs, setSelectedSongs] = useState([]);  // IDs user picked
  const [error, setError] = useState(null);

  // 1) Load *all* songs into the MultiSelect
  useEffect(() => {
    (async () => {
      try {
        // ask for a big page so you get every song
        const wrapper = await fetchSongs(1, "", 1000);
        // unwrap the array from whatever shape fetchSongs gives you:
        const items = Array.isArray(wrapper)
          ...
          : [];
        setAllSongs(
          items.map((s) => ({ value: s._id, label: s.title }))
        );
      } catch (e) {
  ...
  // 2) If editing, preload the artist record
  useEffect(() => {
    if (mode !== "edit") return;
    fetchArtistById(id)
      .then((res) => {
        ...
        setGenre(a.genre);
        setBio(a.bio);
        // if you ever wanted to show existing songs, uncomment:
        // setSelectedSongs((a.songs||[]).map((s) => s._id));
      })
      .catch((e) => setError(e.message));
  }, [mode, id]);

  // 3) Submit: just create/update the artist (no deleting songs)
  const onSubmit = async (e) => {
    e.preventDefault();
    setError(null);
    try {
  ...
  return (
    <PageContainer center>
      ...
      <form onSubmit={onSubmit}>
        ...
        <MultiSelect
          label="Songs"
          placeholder="Select songs (if any)"
          data={allSongs}
          value={selectedSongs}       // optional
          onChange={setSelectedSongs} // optional
          searchable
          mt="md"
        />
