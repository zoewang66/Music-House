...
export default function ArtistForm({ mode }) {
  ...
  const form = useForm({
    ...
    validate: {
      name: (v) => (v.trim().length >= 3 ? null : "Name must be at least 3 characters"),
      genre: (v) => (v.trim() ? null : "Genre is required"),
      bio: (v) => (v.trim().length >= 10 ? null : "Bio must be at least 10 characters"),
      songs: (v) => (v.length > 0 ? null : "Select at least one song"),
    },
  });
