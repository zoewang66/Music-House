...
export default function ArtistForm({ mode }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Fetch page 1 with a high limit so we grab all songs
        const res = await fetchSongs(1, 1000);
        console.log("🎯 ArtistForm fetchSongs response:", res.data);

        // The API nests your array under `items`
        const rawSongs = Array.isArray(res.data.items) ? res.data.items : [];

        console.log("✅ ArtistForm rawSongs:", rawSongs.length, "items");

        // Map into Mantine’s { value, label } shape
        setAllSongs(
          rawSongs.map((s) => ({
            value: s._id,
            label: s.title,
  ...
  // ─── Load Existing Artist (Edit Mode) ───────────────────────────────
  useEffect(() => {
    if (mode !== "edit") return;
    ...
    <PageContainer center>
      ...
      <form onSubmit={onSubmit}>
        ...
        <MultiSelect
          ...
          searchable
          withinPortal // ⬅️ teleport dropdown to body
          zIndex={999} // ⬅️ ensure it floats above other layers
          mt="md"
        />
