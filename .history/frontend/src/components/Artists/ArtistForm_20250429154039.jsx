...
export default function ArtistForm({ mode }) {
  ...
  useEffect(() => {
    const loadAllSongs = async () => {
      try {
        ...
        // now we have allItems (55 songs), map into MultiSelect format
        setAllSongs(
          allItems.map((s) => ({
            value: s._id,
            label: s.title,
  ...
  // ─── If editing, load the existing artist ───────────────────────
  useEffect(() => {
    if (mode !== "edit") return;
    ...
    <PageContainer center>
      ...
      <form onSubmit={onSubmit}>
        ...
        <MultiSelect
          ...
          searchable
          withinPortal // teleport the dropdown out of any clipping container
          zIndex={999} // float it above everything
          mt="md"
        />
